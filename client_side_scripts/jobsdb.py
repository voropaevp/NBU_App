...
class JobsDB(object):
    ...
    def from_csv(cls, path):
        ...
        for line in csv_reader:
            ...
            content[line[0]].append(file_list)
            attempts = dict()
            if line[cls.NUMBER_OF_FILES + 2 + offset] == 0:
                ...
                for i in range(1, attempts_number + 1):
                    ...
                    attempts[i] = attempt
                    offset += 11
            content[line[0]].append(attempts)
            for i in range(offset + 1, len(line) - 1):
                content[line[0]].append(line[i])
            if len(content[line[0]]) == cls.NUMBER_OF_FIELDS:  # padding with None
                content[line[0]].append(None)
                content[line[0]].append(None)
            if len(content[line[0]]) == cls.NUMBER_OF_FIELDS + 1:
                content[line[0]].append(None)
...
data_diff = JobsDBDiff(data,data1)
print data_diff.content_diff
print len(data)
print len(data1)
